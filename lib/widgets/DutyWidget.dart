...
class DutyWidget extends StatelessWidget {
  ...
  final bool _uncolored;

  DutyWidget(this._duty, this._previous, this._statistics, [this._uncolored = false]);
  ...
  Color _getDutyColor() {
    if (!_uncolored && _duty.endTime.loc.difference(DateTime.now()).inMinutes < 0) {
      return Colors.grey;
    }
    if (!_uncolored && _duty.acknowledge) return Colors.redAccent;
    return Colors.white;
  }
